...
interface TaskCardProps {
  ...
  onToggleParts: (taskId: string) => void;
  onUpdateRemarks: (taskId: string, newRemarks: string) => void;
  isInactive?: boolean; // 新增：是否为非当前环节任务
}

const TaskCard: React.FC<TaskCardProps> = ({ task, role, onComplete, onToggleParts, onUpdateRemarks, isInactive = false }) => {
  const [isEditingRemarks, setIsEditingRemarks] = useState(false);
  const isHQ = role === 'HQ_OPERATOR';
  const isAllowedToAdvance = !isHQ && STAGE_PERMISSIONS[task.currentStage].includes(role);
  const isAllowedToToggleParts = !isHQ && (role === 'SPARE_PARTS' || role === 'MANAGER' || role === 'CONSULTANT');
  const isOverdue = Date.now() > task.expectedDeliveryTime;
  const isSmallRepair = task.assessmentAmount < 2000;
  
  return (
    <div className={`bg-white border rounded-[32px] p-5 flex flex-col md:flex-row gap-5 md:items-center transition-all duration-300 shadow-sm 
      ${isInactive ? 'border-slate-100 opacity-40 grayscale pointer-events-none sm:pointer-events-auto' : 'border-blue-100 ring-1 ring-blue-50/50'} 
      ${isOverdue && !isInactive ? 'border-rose-200 ring-rose-50' : ''}`}
    >
      <div className="flex-1 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className={`font-black tracking-tight ${isInactive ? 'text-slate-500 text-xl' : 'text-slate-900 text-3xl'}`}>
              {task.licensePlate}
            </span>
            {!isInactive && (
              <>
          ...
          <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest ${
            isInactive ? 'bg-slate-50 text-slate-400 border border-slate-100' :
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 py-1">
          <div className="space-y-0.5">
            ...
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500">
              ...
              {new Date(task.entryTime).toLocaleString([], {month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit'})}
            </div>
          </div>
          <div className="space-y-0.5">
            <p className="text-[9px] font-black text-slate-400 uppercase">交付时限</p>
            <div className={`flex items-center gap-1.5 text-xs font-black ${isOverdue && !isInactive ? 'text-rose-600' : 'text-slate-500'}`}>
          ...
          <div className="space-y-0.5">
            ...
            <div className={`flex items-center gap-1.5 text-xs font-black ${isInactive ? 'text-slate-400' : 'text-blue-600'}`}>￥{task.assessmentAmount}</div>
          </div>
          {!isInactive && (
            <>
              <div className="space-y-0.5">
        ...
        {!isInactive && (
          <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100 group">
            ...
            {isEditingRemarks ? (
              ...
              <p className={`text-xs font-medium text-slate-600 leading-relaxed ${isHQ ? 'cursor-default' : 'cursor-text'}`} onClick={() => !isHQ && setIsEditingRemarks(true)}>
                {task.remarks || (isHQ ? '暂无备注' : '暂无备注...')}
              </p>
            )}
          </div>
        )}

        {!isInactive && isAllowedToToggleParts && (
      ...
      <div className={isInactive ? 'hidden sm:flex min-w-[120px] flex-col items-center justify-center gap-1' : 'min-w-[180px] md:border-l md:pl-8 border-slate-100'}>
        {isAllowedToAdvance && !isInactive ? (
          <button onClick={() => onComplete(task.id)} className="w-full py-5 rounded-2xl font-black text-sm bg-blue-600 text-white hover:bg-blue-700 shadow-xl shadow-blue-100 flex flex-col items-center gap-1 active:scale-95 transition-transform">
            <CheckCircle2 size={24}/>
            {STAGE_ACTION_LABELS[task.currentStage]}
          </button>
        ) : (
          <div className={`text-center py-5 px-3 rounded-2xl border flex flex-col items-center gap-2 ${isInactive ? 'bg-transparent border-transparent' : 'bg-slate-50 border-slate-100 opacity-60'}`}>
            <Clock size={18} className="text-slate-300" />
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-tight">
              {isInactive ? '其他环节' : isHQ ? '总部观测' : '等待施工'}
            </div>
          </div>
        )}
      </div>
...
const WorkerDashboard: React.FC<Props> = ({ tasks, role, onComplete, onToggleParts, onUpdateRemarks }) => {
  ...
  const sortedTasks = useMemo(() => {
    const activeTasks = tasks.filter(t => t.currentStage !== RepairStage.FINISHED);
    
    if (role === 'METALWORKER' || role === 'PAINTER') {
      return [...activeTasks].sort((a, b) => {
        const aIsMyStage = (role === 'METALWORKER' && a.currentStage === RepairStage.METALWORK) || 
                           (role === 'PAINTER' && a.currentStage === RepairStage.PAINTING);
        const bIsMyStage = (role === 'METALWORKER' && b.currentStage === RepairStage.METALWORK) || 
                           (role === 'PAINTER' && b.currentStage === RepairStage.PAINTING);
        
        if (aIsMyStage && !bIsMyStage) return -1;
        if (!aIsMyStage && bIsMyStage) return 1;
        return b.entryTime - a.entryTime;
      });
    }
    
    return activeTasks.sort((a, b) => b.entryTime - a.entryTime);
  }, [tasks, role]);
      ...
      <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-slate-900 rounded-[22px] flex items-center justify-center text-white shadow-2xl">
            {isHQ ? <Globe size={28}/> : role === 'METALWORKER' ? <Hammer size={28}/> : role === 'PAINTER' ? <Paintbrush size={28}/> : <ShieldAlert size={28}/>}
          </div>
          <div>
            <h2 className="text-2xl font-black text-slate-900">
              {isHQ ? '总部实时监控清单' : role === 'METALWORKER' ? '钣金车间工作台' : role === 'PAINTER' ? '喷漆车间工作台' : role === 'SPARE_PARTS' ? '备件清单' : '理赔业务台'}
            </h2>
            <p className="text-xs font-bold text-slate-400 tracking-wider uppercase">
              共监控到 {sortedTasks.length} 台在修车辆
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        {sortedTasks.length > 0 ? (
          sortedTasks.map(task => {
            const isMyStage = (role === 'METALWORKER' && task.currentStage === RepairStage.METALWORK) || 
                              (role === 'PAINTER' && task.currentStage === RepairStage.PAINTING);
            const isInactive = !isHQ && (role === 'METALWORKER' || role === 'PAINTER') && !isMyStage;
          ...
          <div className="py-24 text-center bg-white rounded-[40px] border border-dashed border-slate-200">
            ...
            <p className="text-slate-400 font-black text-xl">目前车间没有待处理任务</p>
          </div>
        )}
      </div>
    </div>
